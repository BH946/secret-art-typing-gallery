...
기존 하던 프로젝트 "태그" 바꿀만한건 그냥 "데이터목록들 <table> 사용" 이나
"label, input 조합 form 데이터" 정도?? 나머지는 <div> 로 잘 구분하는 정도

+ 타임리프(@{}, |...|) + redirect 잘썻나도 보자(PRG 반드시 확인, 저장후 저장완료 표시..)
+ redirect 의 경우 제공하는 클래스있으니 그 클래스를 꼬 ㄱ사용
...
* 외부 설정과 프로필
  * 특히 외부 파일의 내용을 사용하고 싶을때는 `@ConfigurationProperties` 를 사용
  * **개발모드 실행때 `java -jar external-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod`**
* 타임리프1
  * "태그" 바꿀만한것? 
    * \<table>+\<th>\<td> 등등 -> **이미 div로 잘 만들어놔서 그냥`th:each` + `<th:block>` 만 추가 사용했음**
    * "form 데이터는 label, input 조합"로 바꿈, \<div> 로 구간별 나누는것도 진행.
      * **th:field는 input이나 체크박스에 사용하자 -> form에 쓰면 될듯**
      * 추가로 *{...} 랑 th:object로 사용가능한건 관리하기 쉽게끔 사용하자
  * 타임리프(@{}, |...|)문법 수정(아래 형태로 전부 수정)
    * |...| : `<span th:text="|Welcome to our application, ${user.name}!|">`
    * @{} : 간편) -`th:href="@{|/basic/items/${item.id}|}"`
  * redirect 제공 클래스 반드시 사용
  * PRG 반드시 확인-CHECKLIST, 저장후 저장완료 표시..
* 타임리프2
  * ...
...
* 전시실 세부
  ...
  * "모달의 **추가**" 부분과 작품제작실 만하면 디자인부분은 거진 끝? 리팩토링만 더 빡시게하면될ㄷㅅ 어쨋든 모달의 추가부분 할차례. **form**.

**자꾸 햇갈리는데. "html"별로 디자인 수정 + 타임리프1 부분" 하자는거  **
**이거 다끝나면 타임리프2 하는거임!!**
